...
const Catalog = ({ products, categoryTitle, categoryLink }) => {
	return (
		<div className='py-10 mt-5 bg-slate-50 min-h-screen'>
			<div className='container mx-auto'>
				<h2 className='sd:text-5xl xz:text-3xl font-semibold mb-4 text-center'>
					{categoryTitle}
				</h2>
				<div className='grid sd:grid-cols-4 xz:grid-cols-1 xx:grid-cols-2 sd:gap-4 xz:gap-4 xx:gap-2 mt-10'>
					{products.map((product) => (
						<Link
							href={`/${categoryLink}/${product.id}`}
							key={`${product.title}-${product.id}`}
						>
							<article className="card bg-white rounded-3xl shadow-xl flex flex-col sd:h-[550px] xz:h-[460px] relative">
								{/* Флаг Беларуси */}
								{product.country === "Беларусь" && (
								...
								<div className="card-body sd:p-4 xz:p-2 cursor-pointer flex flex-col justify-between flex-grow">
									<div>
										<p className={`sd:text-base xz:text-xs text-green-500 font-semibold ${product.price ? 'block' : 'hidden'}`}>Цена: {product.price * ExtraCharge} руб.</p>
